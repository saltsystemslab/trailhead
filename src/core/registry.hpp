...
struct NodeProfile {
    ...
    std::vector<std::string> preamble;  // per-node shell lines emitted in sbatch after project preamble
...
struct Registry {
    ...
    // Per-sub-registry sbatch_defaults and setup, keyed by sub_rel path (e.g. "gunrock").
    // Populated by merge_sub_registries. Not serialized.
    std::unordered_map<std::string, SbatchDefaults>            sub_sbatch_defaults;
    std::unordered_map<std::string, std::vector<std::string>>  sub_setups;
};
...
inline NodeProfile node_from_json(const std::string& name, const JsonValue& v) {
    ...
    n.preamble     = v.get_str_array("preamble");
    static const std::vector<std::string> known = {
        "partition","nodelist","gpu_type","nodes","ntasks","cpus_per_task","time","account","rsync_dest","build_dir","cuda_arch","preamble"
    };
    if (v.is_object()) {
...
inline JsonValue node_to_json(const NodeProfile& n) {
    ...
    if (!n.preamble.empty()) {
        JsonArray parr;
        for (const auto& l : n.preamble) parr.push_back(JsonValue(l));
        obj.push_back({"preamble", JsonValue(std::move(parr))});
    }
